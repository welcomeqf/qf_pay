...
public class JwtUtil {
    ...
    public static String createJwt(long ttlMillis, UserLoginQuery user) {
        ...
        JwtBuilder builder = getJwtBuilder (claims, now, subject, signatureAlgorithm, key);
        if (ttlMillis >= 0) {
            setExpDate (nowMillis, ttlMillis, builder);
        }

        return builder.compact();
    }

    public static String createJwt(UserLoginQuery user) {
        //指定签名的时候使用的签名算法，也就是header那部分，jwt已经将这部分内容封装好了。
        SignatureAlgorithm signatureAlgorithm = SignatureAlgorithm.HS256;
    ...
    public static String createJwt(long ttlMillis, UserLoginQuery user, String keys) {
        //指定签名的时候使用的签名算法，也就是header那部分，jwt已经将这部分内容封装好了。
        SignatureAlgorithm signatureAlgorithm = SignatureAlgorithm.HS256;
        ...
        //下面就是在为payload添加各种标准声明和私有声明了
        //这里其实就是new一个JwtBuilder，设置jwt的body
        JwtBuilder builder = getJwtBuilder (claims, now, subject, signatureAlgorithm, keys);
        if (ttlMillis >= 0) {
            setExpDate (nowMillis, ttlMillis, builder);
        }
